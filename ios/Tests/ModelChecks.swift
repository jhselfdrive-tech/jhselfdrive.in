...
@main struct ModelChecks {
  static func main() throws {
    ...
    precondition(
      pickup.bookingMinute.addingTimeInterval(86400).timeIntervalSince(pickup.bookingMinute)
        == 86400)
    precondition(
      ceil(
        returning.bookingMinute.addingTimeInterval(60).timeIntervalSince(pickup.bookingMinute)
          / 86400) == 3)
    ...
    precondition(
      OpsConfig.url(host: "example.supabase.co", scheme: "https")?.host == "example.supabase.co")
    ...
    let roundTrip = try JSONDecoder().decode(
      BookingDetail.self, from: JSONEncoder().encode(booking))
    precondition(roundTrip.id == booking.id && roundTrip.endAt == booking.endAt)

    precondition(BookingPresentation.primary("confirmed") == "Start trip (delivery)")
    precondition(BookingPresentation.primary("ongoing") == "Complete trip (return)")
    precondition(BookingPresentation.primary("cancelled") == nil)
    precondition(BookingPresentation.cards("requested").first == "customer")
    precondition(BookingPresentation.cards("approved").first == "vehicle")
    precondition(BookingPresentation.cards("confirmed").first == "delivery")
    precondition(BookingPresentation.cards("ongoing").first == "returnDue")
    precondition(BookingPresentation.cards("completed").first == "money")
    precondition(BookingPresentation.locked("rejected") && !BookingPresentation.locked("completed"))
    precondition(
      TripCountdown.text(
        start: pickup, end: returning, status: "ongoing",
        now: returning.addingTimeInterval(5 * 3600)) == "Overdue 5h")
    precondition(
      TripCountdown.text(start: pickup, end: returning, status: "completed", now: returning) == nil)

    let binary = Data([0, 255, 13, 10, 22])
    let multipart = Multipart(
      data: binary, filename: "front\"\r\n.jpg", mime: "image/jpeg", fields: ["phase": "delivery"])
    precondition(multipart.data.range(of: binary) != nil)
    let text = String(decoding: multipart.data, as: UTF8.self)
    precondition(
      text.contains("Content-Type: image/jpeg") && text.contains("filename=\"front.jpg\""))
    ...
    context.setFillColor(CGColor(red: 0.93, green: 0.41, blue: 0.3, alpha: 1))
    context.fill(CGRect(x: 0, y: 0, width: 3000, height: 2000))
    let source = NSMutableData()
    let destination = CGImageDestinationCreateWithData(
      source, UTType.png.identifier as CFString, 1, nil)!
    CGImageDestinationAddImage(
      destination, context.makeImage()!,
      [
        kCGImagePropertyGPSDictionary: [
          kCGImagePropertyGPSLatitude: 9.37, kCGImagePropertyGPSLongitude: 78.83,
        ]
      ] as CFDictionary)
    ...
    print(
      "Swift checks passed: status priorities/actions, countdowns, rental minute precision, old detail compatibility, dates, cache coding, multipart bytes, JPEG size/type and metadata stripping."
    )
  }
}
