...
import { composeMessage } from "@/lib/messages/templates";
...
export function EnquiryRow({ enquiry }: { enquiry: Enquiry }) {
  ...
  const message = composeMessage("enquiry_followup", { customerName: customer?.full_name, carLabel: car?.name || enquiry.car_slug, startAt: enquiry.pickup_date, endAt: enquiry.return_date });
  return <tr><td><div className="admin-customer-cell"><span className="admin-avatar">{initials}</span><div><strong>{customer?.full_name || "Unnamed customer"}</strong><small>{customer?.phone}</small></div></div></td><td><strong>{car?.name || enquiry.car_slug}</strong><small style={{display:"block",color:"var(--muted)",marginTop:3}}>{enquiry.pickup_date} → {enquiry.return_date}</small></td><td>{new Date(enquiry.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</td><td><StatusSelect id={enquiry.id} status={enquiry.status} /></td><td><div className="admin-row-actions"><a href={whatsAppUrl(customer?.phone || "", message.body)} target="_blank" rel="noreferrer" aria-label="WhatsApp customer"><MessageCircle size={15} /></a><a href={`tel:${customer?.phone}`} aria-label="Call customer"><Phone size={15} /></a>{enquiry.status !== "converted" ? <Link className="admin-convert" href={`/admin/bookings?enquiry=${enquiry.id}`}><CalendarPlus size={14} /> Convert</Link> : null}</div></td></tr>;
}
