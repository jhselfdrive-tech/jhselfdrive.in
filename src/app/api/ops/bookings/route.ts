...
const querySchema = z.object({ status: z.enum([...BOOKING_STATUSES, 'all', 'active', 'upcoming']).default('all'), search: z.string().max(100).optional(), limit: z.coerce.number().int().min(1).max(250).default(50), from: timestamp.optional(), to: timestamp.optional(), cursor: z.string().max(300).optional() });
