...
export default async function AdminDashboardPage() {
  ...
  return <>
    <div className="admin-page-head"><div><span className="admin-overline">Live business pulse</span><h1>Good decisions start here.</h1><p>Demand, conversion, fleet risk and revenue at a glance.</p></div><div className="admin-head-actions"><Link className="admin-primary-button" href="/admin/bookings">View bookings <ArrowUpRight size={14} /></Link></div></div>
    <AlertsPanel alerts={alerts} />
    <section className="admin-metrics">
      <MetricCard label="Requests to review" value={String(metrics.pendingBookings)} detail={metrics.pendingBookings ? "Booking requests awaiting review" : "All requests reviewed"} icon={Inbox} color="#fff0d8" ink="#a46510" />
      ...
      <MetricCard label="Conversion rate" value={`${metrics.conversionRate}%`} detail="Requests that reached approval" icon={TrendingUp} color="#e0f1e8" />
    </section>
    <Suspense fallback={<GaTrafficStripSkeleton />}><GaTrafficStrip days={7} /></Suspense>
    <section className="admin-dashboard-grid">
      <article className="admin-card"><div className="admin-card-head"><div><h2>Booking trend</h2><span className="admin-card-subtitle">Weekly demand over the last 8 weeks</span></div><Activity size={18} color="var(--teal)" /></div><TrendChart data={metrics.trend} /></article>
      ...
      <article className="admin-card"><div className="admin-card-head"><div><h2>Top requested cars</h2><span className="admin-card-subtitle">What customers want most</span></div></div><RankList items={topCars} emptyText="No booking requests yet." /></article>
      ...
      <article className="admin-card"><div className="admin-card-head"><div><h2>Traffic sources</h2><span className="admin-card-subtitle">Where visitors come from</span></div><Link className="admin-card-link" href="/admin/traffic">All traffic <ArrowUpRight size={13} /></Link></div><RankList items={metrics.sources} emptyText="Source data will appear with site traffic." accent="linear-gradient(90deg,#ed684d,#ffc162)" capitalize /></article>
    </section>
  </>;
}
