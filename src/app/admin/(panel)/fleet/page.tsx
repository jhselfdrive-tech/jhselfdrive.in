...
import { AlertTriangle, CarFront, Database, EyeOff, Filter, Gauge, ShieldCheck } from "lucide-react";
...
export default async function FleetPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const { vehicles, schemaReady } = await listVehicles({ status, category });
  ...
  return <>
    <div className="admin-page-head">
      ...
      <div className="admin-head-actions"><VehicleDialog /></div>
    </div>
    {!schemaReady ? <div className="admin-notice"><Database size={18} /><div><strong>Fleet database setup is still required</strong><p>Apply <code>supabase/migrations/0006_vehicle_bookings.sql</code> to enable photos, per-car rates and date-based customer booking.</p></div></div> : null}
      ...
      {vehicles.map((vehicle) => {
        ...
        return <Link className="admin-vehicle-card" href={`/admin/fleet/${vehicle.id}`} key={vehicle.id}>
          <div className="admin-vehicle-photo">
            ...
            {vehicle.photoUrl ? <img src={vehicle.photoUrl} alt="" loading="lazy" /> : <span className="admin-vehicle-photo-empty"><CarFront size={26} /> No photo</span>}
            <span className={`admin-fleet-status admin-fleet-${vehicle.status}`}>{vehicle.status}</span>
            {!vehicle.is_bookable ? <span className="admin-vehicle-hidden"><EyeOff size={11} /> Hidden</span> : null}
          </div>
          ...
          <span className="admin-vehicle-rate">₹{Number(vehicle.day_rate).toLocaleString("en-IN")}/day · {vehicle.photoCount} photo{vehicle.photoCount === 1 ? "" : "s"}</span>
